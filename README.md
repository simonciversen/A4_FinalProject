...
The usecase of the project is primarily for the contractor to present for a client where the largest costs lie, and preferably come with alternative methods of reducing the cost after analysis of the output data. The tool will also create a quick way of comparing the the same structure made with different materials, say if a client wants a main structure buildt with CLT instead of conrete. Secondary it would be useful to apply this tool in interactions with subcontractors to see what costs one could expect if a contractor were to outsource parts of the building process, which is rather common.
...
#### Slabs:
The model has differentiated between the loadbearing slabs and the floor finishes in the model. That being said it has classified all horisontal components realting to the slabs as being loadbearing, meaning such as ceramic floor finishes are also defined as loadbearing. We have taken this into account and only shown the three structural categorial components present in the model and differentiated each material with their thickness. 
